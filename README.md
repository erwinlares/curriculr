...
Open cv-data-template.xlsx and fill in the profile sheet.
Replace placeholder.png with your own profile photo.
Call create_cv(data = 'cv-data-template.xlsx', photo = 'your-photo.png')
...
This writes `CV.pdf` next to your workbook. Open it to review the output.
To update your CV, edit the workbook and call `create_cv()` again — no
flags needed, since the intermediate files are managed automatically.
...
Multi-row entries — positions with more than one responsibility listed in the
`detail` column — are automatically aggregated into a single header with an
indented bulleted list. The same title, unit, dates, and location appearing
on multiple rows produces one entry in the rendered PDF, with each `detail`
value as a separate bullet. Row order in the workbook controls bullet order.
This applies to all sections, not just experience.
...
- `overwrite` -- whether to overwrite scaffold files (template workbook and
  placeholder image) when running in scaffold mode. Has no effect in render
  mode -- intermediate files are written to a temporary directory and never
  appear in the workbook folder. Defaults to `FALSE`.
...
and rendered as a bulleted list. `include_in_resume` is a boolean checkbox
column -- check a row to include it when rendering with `variant = "resume"`.
...
location across multiple rows) are now automatically aggregated into a single
header with an indented bulleted list. Previously each row produced a
separate repeated header. The intermediate `CV.qmd` is now written to a
temporary directory rather than the workbook folder -- it is never visible
to the user and the `overwrite` argument no longer affects render mode.

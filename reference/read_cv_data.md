...
named character vector for convenient scalar access. The `sections`
sheet is returned as a data frame in the order the rows appear in the
workbook — row order controls section render order and must not be
sorted.
...
fields as `cv$profile[["first_name"]]`. Access the sections control
sheet as `cv$sections`.
...
and `sections` sheets are exempt from sorting.
...
sheet must contain at minimum `section` and `label` columns.

...
- [`cv_render_section()`](https://erwinlares.github.io/curriculr/reference/cv_render_section.md)
  now aggregates multi-row entries automatically. Rows sharing the same
  `title`, `unit`, `startMonth`, `startYear`, `endMonth`, `endYear`, and
  `where` values are treated as a single entry: one header renders at
  the top with the date and location right-aligned, and each `detail`
  value appears as an indented bullet beneath it. Single-detail entries
  continue to render exactly as before. Row order in the workbook
...
- [`create_cv()`](https://erwinlares.github.io/curriculr/reference/create_cv.md)
  now writes the intermediate `CV.qmd` to a temporary directory rather
  than the workbook folder. The file is never visible to the user and is
  cleaned up automatically. As a result, `overwrite` no longer has any
  effect in render mode – re-running
  [`create_cv()`](https://erwinlares.github.io/curriculr/reference/create_cv.md)
  on an existing project requires no flags.
...
- Fixed bullet spacing in multi-responsibility CV entries. An earlier
  ...
  rather than from the package. Spacing is now set explicitly, which
  also makes rendered output consistent across different rendering
  environments. Thanks for [@orestesI](https://github.com/orestesI) for
  the bug report

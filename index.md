...
Open cv-data-template.xlsx and fill in the profile sheet. Replace
placeholder.png with your own profile photo. Call create_cv(data =
‘cv-data-template.xlsx’, photo = ‘your-photo.png’)
...
This writes `CV.pdf` next to your workbook. Open it to review the
output. To update your CV, edit the workbook and call
[`create_cv()`](https://erwinlares.github.io/curriculr/reference/create_cv.md)
again — no flags needed, since the intermediate files are managed
automatically.
...
Multi-row entries — positions with more than one responsibility listed
in the `detail` column — are automatically aggregated into a single
header with an indented bulleted list. The same title, unit, dates, and
location appearing on multiple rows produces one entry in the rendered
PDF, with each `detail` value as a separate bullet. Row order in the
workbook controls bullet order. This applies to all sections, not just
experience.
...
Re-render after editing the `theme` sheet to see the changes.
...
- `overwrite` – whether to overwrite scaffold files (template workbook
  and placeholder image) when running in scaffold mode. Has no effect in
  render mode – intermediate files are written to a temporary directory
  and never appear in the workbook folder. Defaults to `FALSE`.
...
Iterates over a CV data frame and writes each entry as a formatted Typst
block. Rows sharing the same title, unit, dates, and location are
automatically grouped into a single entry with a bulleted detail list.
...
`detail` holds the entry description or, for positions with multiple
responsibilities, one responsibility per row. Rows sharing the same
`title`, `unit`, dates, and `where` values are treated as a single entry
and rendered as a bulleted list. `include_in_resume` is a boolean
checkbox column – check a row to include it when rendering with
`variant = "resume"`. Leave cells blank rather than typing `NA`.
...
**v0.3.1 to v0.3.2** – multi-row entries (same title, unit, dates, and
location across multiple rows) are now automatically aggregated into a
single header with an indented bulleted list. Previously each row
produced a separate repeated header. The intermediate `CV.qmd` is now
written to a temporary directory rather than the workbook folder – it is
never visible to the user and the `overwrite` argument no longer affects
render mode.

**v0.2.0 to v0.3.0** – `photo = NULL` now produces a no-photo layout
instead of falling back to a placeholder. Workbooks need an
`include_in_resume` column on every section sheet. An optional `theme`
sheet controls fonts, colors, and page layout. `openxlsx2` replaces
`readxl`.

**v0.1.0 to v0.2.0** –
[`create_cv()`](https://erwinlares.github.io/curriculr/reference/create_cv.md)
gained scaffold mode (no arguments) and render mode (with arguments).
Workbooks need a `sections` sheet to control which sections appear and
in what order.
...
curriculr shares an author and design philosophy with three other R
packages focused on reproducible research workflows:
...
conventions: small focused functions, data-driven workflows, and Quarto
